...
public class XmlStreamWriter extends Writer
{
    ...
    {
        StringBuffer stringbuffer = xmlPrologWriter.getBuffer();
        int k;
        int l;
        Matcher matcher;
        if (j + stringbuffer.length() > 4096)
        {
...
_L3:
        l = stringbuffer.indexOf("?>");
        if (l <= 0) goto _L6; else goto _L5
_L5:
        matcher = ENCODING_PATTERN.matcher(stringbuffer.substring(0, l));
        if (matcher.find())
        {
            encoding = matcher.group(1).toUpperCase();
            encoding = encoding.substring(1, -1 + encoding.length());
        } else
        {
