    ...
    implements StringEncoder
{
    ...
    private String cleanInput(String s)
    {
        ...
        if (s != null)
        {
            ...
            {
                return s1.toUpperCase(Locale.ENGLISH);
            }
        }
        return null;
    }
...
_L3:
        boolean flag1;
        flag1 = isVowel(charAt(s, i - 2));
        flag = false;
        if (flag1) goto _L4; else goto _L5
_L5:
        boolean flag2;
        flag2 = contains(s, i - 1, 3, "ACH");
        flag = false;
        if (!flag2) goto _L4; else goto _L6
_L6:
        boolean flag3;
        char c = charAt(s, i + 2);
        if (c != 'I' && c != 'E')
        ...
        flag3 = contains(s, i - 2, 6, "BACHER", "MACHER");
        flag = false;
...
_L7:
    ...
    {
        return contains(s, 0, 4, "VAN ", "VON ") || contains(s, 0, 3, "SCH") || contains(s, i - 2, 6, "ORCHES", "ARCHIT", "ORCHID") || contains(s, i + 2, 1, "T", "S") || (contains(s, i - 1, 1, "A", "O", "U", "E") || i == 0) && (contains(s, i + 2, 1, L_R_N_M_B_H_F_V_W_SPACE) || i + 1 == -1 + s.length());
    }

    private boolean conditionL0(String s, int i)
    {
        if (i == -3 + s.length() && contains(s, i - 1, 4, "ILLO", "ILLA", "ALLE"))
        {
            return true;
        }
        return (contains(s, -2 + s.length(), 2, "AS", "OS") || contains(s, -1 + s.length(), 1, "A", "O")) && contains(s, i - 1, 4, "ALLE");
    }

    private boolean conditionM0(String s, int i)
    {
        while (charAt(s, i + 1) == 'M' || contains(s, i - 1, 3, "UMB") && (i + 1 == -1 + s.length() || contains(s, i + 2, 2, "ER"))) 
        {
            return true;
        }
        return false;
...
_L4:
        if (!s1.equals(as[j1])) goto _L6; else goto _L5
_L5:
        flag = true;
_L2:
        return flag;
...
_L7:
    ...
    private int handleC(String s, DoubleMetaphoneResult doublemetaphoneresult, int i)
    {
        ...
        {
            return handleCH(s, doublemetaphoneresult, i);
        }
...
_L3:
    ...
    {
        if (i == -1 + s.length() && !flag && contains(s, i - 2, 2, "IE") && !contains(s, i - 4, 2, "ME", "MA"))
        {
    ...
    private int handleS(String s, DoubleMetaphoneResult doublemetaphoneresult, int i, boolean flag)
    {
        ...
        if (i == -1 + s.length() && contains(s, i - 2, 2, "AI", "OI"))
        {
    ...
    private int handleW(String s, DoubleMetaphoneResult doublemetaphoneresult, int i)
    {
        ...
        if (i == -1 + s.length() && isVowel(charAt(s, i - 1)) || contains(s, i - 1, 5, "EWSKI", "EWSKY", "OWSKI", "OWSKY") || contains(s, 0, 3, "SCH"))
        {
    ...
    private int handleX(String s, DoubleMetaphoneResult doublemetaphoneresult, int i)
    {
        ...
        if (i != -1 + s.length() || !contains(s, i - 3, 3, "IAU", "EAU") && !contains(s, i - 2, 2, "AU", "OU"))
        {
            doublemetaphoneresult.append("KS");
        }
...
label0:
    ...
    {
        String s1 = cleanInput(s);
        if (s1 == null)
        {
            return null;
        }
        boolean flag1 = isSlavoGermanic(s1);
        int i;
        ...
        do
        {
            if (!doublemetaphoneresult.isComplete() && i <= -1 + s1.length())
            {
                switch (s1.charAt(i))
                {
                default:
                ...
                case 67: // 'C'
                    i = handleC(s1, doublemetaphoneresult, i);
                    break;

                case 68: // 'D'
                    i = handleD(s1, doublemetaphoneresult, i);
                    break;
                ...
                case 71: // 'G'
                    i = handleG(s1, doublemetaphoneresult, i, flag1);
                    break;

                case 72: // 'H'
                    i = handleH(s1, doublemetaphoneresult, i);
                    break;

                case 74: // 'J'
                    i = handleJ(s1, doublemetaphoneresult, i, flag1);
                    break;
                ...
                case 76: // 'L'
                    i = handleL(s1, doublemetaphoneresult, i);
                    break;
                ...
                case 80: // 'P'
                    i = handleP(s1, doublemetaphoneresult, i);
                    break;
                ...
                case 82: // 'R'
                    i = handleR(s1, doublemetaphoneresult, i, flag1);
                    break;

                case 83: // 'S'
                    i = handleS(s1, doublemetaphoneresult, i, flag1);
                    break;

                case 84: // 'T'
                    i = handleT(s1, doublemetaphoneresult, i);
                    break;
                ...
                case 87: // 'W'
                    i = handleW(s1, doublemetaphoneresult, i);
                    break;

                case 88: // 'X'
                    i = handleX(s1, doublemetaphoneresult, i);
                    break;

                case 90: // 'Z'
                    i = handleZ(s1, doublemetaphoneresult, i, flag1);
                    break;
                }
            } else
            if (flag)
