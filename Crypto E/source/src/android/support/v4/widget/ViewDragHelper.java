...
public class ViewDragHelper
{
    ...
    private boolean checkNewEdgeDrag(float f, float f1, int i, int j)
    {
        ...
        {
            if (f2 < f3 * 0.5F && mCallback.onEdgeLock(j))
            {
...
_L6:
    ...
    private void dragTo(int i, int j, int k, int l)
    {
        ...
        if (k != 0)
        {
            k1 = mCallback.clampViewPositionHorizontal(mCapturedView, i, k);
            mCapturedView.offsetLeftAndRight(k1 - i1);
        } else
        {
            k1 = i;
        }
        if (l != 0)
        {
            l1 = mCallback.clampViewPositionVertical(mCapturedView, j, l);
            mCapturedView.offsetTopAndBottom(l1 - j1);
        } else
        {
            l1 = j;
        }
        if (k != 0 || l != 0)
        {
            int i2 = k1 - i1;
            int j2 = l1 - j1;
            mCallback.onViewPositionChanged(mCapturedView, k1, l1, i2, j2);
        }
    }
...
_L2:
        ...
        int k3 = MotionEventCompat.getPointerId(motionevent, 0);
        View view1 = findTopChildUnder((int)f10, (int)f11);
        saveInitialMotion(f10, f11, k3);
        tryCaptureViewForDrag(view1, k3);
        int l3 = mInitialEdgesTouched[k3];
        if ((l3 & mTrackingEdges) != 0)
        {
            mCallback.onEdgeTouched(l3 & mTrackingEdges, k3);
            return;
        }
          goto _L1
_L6:
        ...
        float f9;
        int j3;
        i3 = MotionEventCompat.getPointerId(motionevent, k);
        f8 = MotionEventCompat.getX(motionevent, k);
        f9 = MotionEventCompat.getY(motionevent, k);
        saveInitialMotion(f8, f9, i3);
        if (mDragState != 0)
        {
            continue; /* Loop/switch isn't completed */
        }
        tryCaptureViewForDrag(findTopChildUnder((int)f8, (int)f9), i3);
        j3 = mInitialEdgesTouched[i3];
        if ((j3 & mTrackingEdges) == 0) goto _L1; else goto _L8
_L8:
        mCallback.onEdgeTouched(j3 & mTrackingEdges, i3);
        return;
        if (!isCapturedViewUnder((int)f8, (int)f9)) goto _L1; else goto _L9
_L9:
        tryCaptureViewForDrag(mCapturedView, i3);
        return;
_L4:
        int l1;
        if (mDragState == 1)
        {
            ...
            float f6 = MotionEventCompat.getX(motionevent, j2);
            float f7 = MotionEventCompat.getY(motionevent, j2);
            int k2 = (int)(f6 - mLastMotionX[mActivePointerId]);
            int l2 = (int)(f7 - mLastMotionY[mActivePointerId]);
...
_L14:
        if (i >= l1) goto _L11; else goto _L10
_L10:
        int i2;
        float f2;
        float f3;
        float f4;
        float f5;
        i2 = MotionEventCompat.getPointerId(motionevent, i);
        f2 = MotionEventCompat.getX(motionevent, i);
        f3 = MotionEventCompat.getY(motionevent, i);
        f4 = f2 - mInitialMotionX[i2];
        f5 = f3 - mInitialMotionY[i2];
        reportNewEdgeDrags(f4, f5, i2);
...
_L11:
        ...
        return;
_L12:
        if (checkTouchSlop(view = findTopChildUnder((int)f2, (int)f3), f4, f5) && tryCaptureViewForDrag(view, i2)) goto _L11; else goto _L13
_L13:
        i++;
          goto _L14
_L7:
        int l = MotionEventCompat.getPointerId(motionevent, k);
...
_L17:
        int k1 = MotionEventCompat.getPointerId(motionevent, i);
...
_L8:
        ...
        float f = MotionEventCompat.getX(motionevent, l);
...
_L11:
        ...
        if (mDragState == 1 || flag && tryCaptureViewForDrag(view, i1)) goto _L9; else goto _L12
_L12:
        l++;
          goto _L13
_L6:
